...
class actualvsForecast_testing( unittest.TestCase ):
    def test_request( self ):
        result = runner.invoke(cli.main, ['energy_group001', 'Login', '--username', 'admin', '--passw', 'nimda123'])
        token = result.output
        result = runner.invoke(cli.main, ['energy_group001', 'ActualvsForecast', '--area', 'Slovakia', '--timeres', 'PT15M', '--date', '2018-01-01'])
